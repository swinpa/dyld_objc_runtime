...
_dispatch_once_wait(dispatch_once_gate_t dgo)
{
	...
	for (;;) {
		/*
		 os_atomic_rmw_loop用于从操作系统底层获取状态，使用 os_atomic_rmw_loop_give_up 来执行返回操作。
		 即不停查询 &dgo->dgo_once 的值，若变为DLOCK_ONCE_DONE，则调用 os_atomic_rmw_loop_give_up(return); 退出等待。
		 */
