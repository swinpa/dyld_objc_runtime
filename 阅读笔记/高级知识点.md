...
1. 渲染时机
	...
	Core Animation在主线程的RunLoop中注册监听了before waiting事件，在收到该事件时对应的回
...
####内存leaks 检测[参考1](http://hchong.net/2020/03/11/iOS%E5%86%85%E5%AD%98%E6%B3%84%E6%BC%8F%E6%A3%80%E6%9F%A5-%E5%8E%9F%E7%90%86/)  [参考1](https://xibhe.com/2019/05/27/iOS%E5%86%85%E5%AD%98%E6%B3%84%E6%BC%8F%E6%A3%80%E6%B5%8B/)
	...
	1. 检测步骤
		...
		3. Instruments Leaks (动态检测)
			...
			Leaks 工具检测流程：
			1. 启动应用程序： 在 Instruments 中启动 Leaks 工具，并选择要分析的应用程序。
			2. 记录内存分配和释放事件： Leaks 工具会记录应用程序运行期间的所有内存分配和释放事件。
			3. 分析引用计数： 工具会跟踪对象的引用计数变化。当一个对象的引用计数不为零，但应用程序不再有任何对该对象的引用时，可能发生内存泄漏。
			4. 生成报告： Leaks 工具会生成一份报告，列出可能的内存泄漏，包括泄漏对象的类型、堆栈跟踪信息等。
			5. 符号化（Symbolication）： Instruments Leaks 工具还会尽可能地对报告中的地址进行符号化，将其转换为易读的函数名和文件路径，以便开发者更容易定位问题的源头。
			6. 注意事项： Instruments Leaks 工具的结果可能包含一些误报，因为它主要依赖引用计数来检测内存泄漏。在使用工具时，开发者需要注意一些特殊情况，例如循环引用、Core Foundation 对象的内存管理等
...
* 检测方案[iOS卡顿监测方案总结](https://juejin.cn/post/6844903944867545096)
...
* [Matrix-iOS 卡顿监控](https://cloud.tencent.com/developer/inventory/6249/article/1427933)
