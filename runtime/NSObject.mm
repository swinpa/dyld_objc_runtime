...
struct SideTable {
    /*
     ...
     具体来说，如果一个低优先级的线程获得锁并访问共享资源，这时一个高优先级的线程也尝试获得这个锁，它会处于 spin lock 的忙等状态从而占用大量
     CPU。此时低优先级线程无法与高优先级线程争夺 CPU 时间，从而导致任务迟迟完不成、无法释放 lock。这并不只是理论上的问题，libobjc
     已经遇到了很多次这个问题了，于是苹果的工程师停用了 OSSpinLock。
     
     
     也就是在低优先级的线程中加锁获取资源，这时高优先级的线程进来加锁，抢占了CPU资源，导致底优先级线程迟迟没释放锁，
     而高优先级在获取锁时，由于低优先级已经上锁，却没释放，导致高优先级线程一直等待该锁
     有点类似于死锁状态
     
     */
    /*
     这里底层已经修改成了class mutex_tt : nocopy_t {
