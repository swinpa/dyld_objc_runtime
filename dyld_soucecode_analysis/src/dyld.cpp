...
#if __x86_64__
static bool							sHaswell = false; //如果是iphone就是false，x86_64是true
#endif
...
//检测系统中的dylib版本与加载的dylib版本的优先级
//
// 先获取dylibfile的版本在获取系统安装的dylib的版本。
// 如果当前加载的版本大于系统中的版本。
// 
...
//给环境变量sEnv通过 appendParsedColonList 赋值
...
// 为了安全起见，programs 设置忽略DYLD_* 环境变量的标识符
// Additionally, the DYLD_* enviroment variables are removed
// 另外将DYLD_* 环境变量 从环境变量中删除
...
static void checkSharedRegionDisable()
{
	...
	if ( sMainExecutable->overlapsWithAddressRange((void*)(uintptr_t)SHARED_REGION_BASE, (void*)(uintptr_t)(SHARED_REGION_BASE + SHARED_REGION_SIZE)) ) {
		//share_region_base到SHARED_REGION_BASE + SHARED_REGION_SIZE之间是否存在存在和segments重合的部分
		gLinkContext.sharedRegionMode = ImageLoader::kDontUseSharedRegion;
...
// 主要是检测mach-o文件的cputype和cpusubtype是否与当前系统兼容
...
{
	// try mach-o loader
	if ( isCompatibleMachO((const uint8_t*)mh, path) ) {//检测是否合法,检测mach-o文件的cputype和cpusubtype是否与当前系统兼容
...
// try with and without image 
// 尝试有后缀已经没有后缀的image，_debug,_profile
...
// expand @ variables
// 使用绝对的路径path去加载
// 对 "@executable_path/"，"@loader_path/"，"@rpath/"三个不同的路径做了不同的路径修正
// 最后调用loadPhase4
...
// try search paths
// 根据frameworkPaths[]和libraryPaths[]两个参数生成新的路径去试图调用
// loadPhase4
static ImageLoader* loadPhase2(const char* path, const char* orgPath, const LoadContext& context, 
							   const char* const frameworkPaths[], const char* const libraryPaths[], 
...
int openSharedCacheFile()
{
	...
	strlcpy(path, sSharedCacheDir, MAXPATHLEN); //"/var/db/dyld/"
	strlcat(path, "/", MAXPATHLEN);
#if __x86_64__
	if ( sHaswell ) {
		strlcat(path, DYLD_SHARED_CACHE_BASE_NAME ARCH_NAME_H, MAXPATHLEN); //"dyld_shared_cache_"
...
static void mapSharedCache()
{
	...
	if ( _shared_region_check_np(&cacheBaseAddress) == 0 ) {
		...
		if ( (header->mappingOffset >= 0x48) && (header->slideInfoSize != 0) ) {
			// 具有slideinfo的时候需要对sSharedCacheSlide做一次修正。
			...
			if ( ::read(fd, firstPages, 8192) == 8192 ) {
				...
				if ( strcmp(header->magic, magic) == 0 ) {
					...
					if ( fstat(fd, &stat_buf) == 0 ) {
						goodCache = true;
						int i=0;
						//这里是从/var/db/dyld/dyld_shared_cache_ 上读取到的数据，拷贝到了栈上的mappings数组。
						...
						// header->codeSignatureSize的值为0说明签名数据一直到文件结束
						if ( signatureSize == 0 )
...
#if SUPPORT_VERSIONED_PATHS
//读取文件的第一个page（4096字节）支持mach-o和fat格式
...
// 获取dylib的版本与安装时的名字
// 
// 读取mh信息，根据loadcommnds遍历寻找lc_id_dylib字段，提取version和installname的字符串

static bool getDylibVersionAndInstallname(const char* dylibPath, uint32_t* version, char* installName)
{
	uint8_t firstPage[4096];
	const macho_header* mh = (macho_header*)firstPage;
	//读取第一个page就是读取macho的头部信息，系统优化将头部信息放在内存的第一页中
	if ( !readFirstPage(dylibPath, firstPage) ) {
	#if DYLD_SHARED_CACHE_SUPPORT
		// If file cannot be read, check to see if path is in shared cache
		//如果读取第一个page失败了，并且系统支持dylib的cache，就从共享的cache文件中去查找一次
		const macho_header* mhInCache;
		const char*			pathInCache;
		long				slideInCache;
...
/**
 ...
 @param mainExecutableMH app 的 mach header
 @param mainExecutableSlide <#mainExecutableSlide description#>
 @param argc <#argc description#>
 @param argv <#argv description#>
 @param envp <#envp description#>
 @param apple <#apple description#>
 @param startGlue <#startGlue description#>
		...
		uintptr_t* startGlue)
{
	...
	//根据名字"DYLD_ROOT_PATH" 从envp（环境列表）中获取环境变量
    ...
    sProcessIsRestricted = processRestricted(mainExecutableMH,
											 ...
											 &sProcessRequiresLibraryValidation); //检查是否受限
	/*
	 检查环境变量，比如sEnv.DYLD_INSERT_LIBRARIES 就在
	...
	try {
...
#if TARGET_IPHONE_SIMULATOR //版本检测与运行环境。
	#if TARGET_OS_WATCH || TARGET_OS_TV
		// disable error during bring up of these simulators
	#else
		...
		if	( sEnv.DYLD_INSERT_LIBRARIES != NULL ) {
			...
			{
				/*
				 加载各种依赖的动态库,并添加到sAllImages容器中
				 loadInsertedDylib()
				 	->|load()
				 		->|checkandAddImage()
				 			->|addImage() 这样最终加到sAllImages容器中
				 */
		...
		//链接的时候回调通知runtime image 的 mapped 状态
		...
		/*
		 执行所有image的初始化函数，回调通知runtime，notifyBatch(dyld_image_state_initialized);
		 也就是回调到了runtime的load_images 方法来执行后面的+load 方法
		*/
		initializeMainExecutable();
	#endif
		// find entry point for main executable
		// 找到函数的入口点，也就是应用的main函数
