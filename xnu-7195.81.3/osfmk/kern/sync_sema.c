 ...
 *
 *		Decrements(衰减) the semaphore count by one.
 *      将信号量计数减1
 *      If the count is negative after the decrement,
 *      如果计数在减量后为负数
 *      the calling thread blocks (possibly at a continuation and/or with a timeout).
 *      调用线程阻塞
 *
 *	Assumptions:
 *		The reference
